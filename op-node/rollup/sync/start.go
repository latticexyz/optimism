...
func FindL2Heads(ctx context.Context, cfg *rollup.Config, l1 L1Chain, l2 L2Chain, lgr log.Logger, syncCfg *Config) (result *FindHeadsResult, err error) {
	...
	for {
		...
		if result.Unsafe == (eth.L2BlockRef{}) {
			...
			if n.L1Origin.Number+(MaxReorgSeqWindows*cfg.ReorgWindowSize()) < prevUnsafe.L1Origin.Number {
		...
		if n.Number <= result.Safe.Number && n.L1Origin.Number+cfg.ReorgWindowSize() < highestL2WithCanonicalL1Origin.L1Origin.Number && n.SequenceNumber == 0 {
			ready = true
		}
