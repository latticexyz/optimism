...
func (l *BatchSubmitter) publishToAltDAAndL1(txdata txData, queue *txmgr.Queue[txRef], receiptsCh chan txmgr.TxReceipt[txRef], daGroup *errgroup.Group) {
	// sanity checks
	// TODO: disable this log for generic commitments?
	...
	goroutineSpawned := daGroup.TryGo(func() error {
		...
		var calldata []byte
		if len(comms) > 1 {
			// TODO: check that these are actually generic keccak256 commitments
			batchedComm := []byte{byte(altda.Keccak256DALayer)}
			for _, c := range comms {
				// Strip the CommitmentType and the DALayer byte
				// TODO: this is a workaround as there is no way currently of getting the underlying bytes
				batchedComm = append(batchedComm, c.Encode()[2:]...)
			}
...
func (l *BatchSubmitter) prepareAltDAInputs(txdata txData) [][]byte {
	// TODO: differentiate depending on a config flag?
